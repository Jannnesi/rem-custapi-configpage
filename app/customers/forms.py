...
class CustomerForm(FlaskForm):
    ...
    destination_container   = StringField('Destination Container', validators=[DataRequired(), Length(max=100)])
    ...
    file_encoding           = SelectField(
                                'File Encoding',
                                choices=[('utf-8','UTF-8'),('iso-8859-1','ISO-8859-1'),('windows-1252','Windows-1252')],
                                validators=[DataRequired()]
                            )
    extra_columns           = FieldList(
                                FormField(ExtraColumnForm),
                                min_entries=0,
                                label='Extra Columns',
                                validators=[Optional()]
                            )
    exclude_columns       = FieldList(
                              StringField('Column to Exclude',
